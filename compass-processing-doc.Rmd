...
## Processing
Due to the large amount of data COMPASS-FME generates, it can be difficult to handle.  Using the HPC and R scripts, COMPASS-FME data is summarized into a more manageable size.

### HPC
High Processing Computing (HPC), is a way to processing data at high speeds.  It can be done through computer clustering.  A computer cluster is a group of computers that work together.  It speeds up the processing time because they work in parallel.  Because of the amount of data that needed to be summarized, clustering computing was needed to run the script quickly.
...
- **NO FIGURES YET**
- *Explain HPC, edit writing*

### Summarizing script
To process the data, two R scripts are used.  The first script, called send_dir_path, sends each csv file in a folder to a second script called compute_na_sd.  This script summarizes the data by grouping data points of the same date and research name.  The data is then summarized into the following columns *(I might change the bullet points to a table)*: 
...
The new dataframe is then returned to send_dir_path.  Once all the dataframes created from the folder are returned, they are combined and exported into a new csv file.
...
C:/Users/hart187/output_dir_temp/example_output_1_10_CRC.csv
PTR 2023 gw_temperature
C:/Users/hart187/output_dir_temp/example_output_20_10_PTR.csv
----------
